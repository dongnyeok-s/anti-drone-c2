...
export class SensorFusion {
  ...
  private tracks: Map<string, ExtendedFusedTrack> = new Map();
  private droneIdToTrackId: Map<string, string> = new Map();
  private basePosition: TrackPosition;

  // 센서 융합 모드
  private fusionMode: FusionMode = 'WEIGHTED_AVG';

  // Extended Kalman Filter (EKF 모드에서 사용)
  private ekf: ExtendedKalmanFilter;

  constructor(basePosition: TrackPosition, config: Partial<FusionConfig> = {}, fusionMode: FusionMode = 'WEIGHTED_AVG') {
    ...
    // EKF 초기화
    this.ekf = new ExtendedKalmanFilter(basePosition, DEFAULT_KALMAN_CONFIG);
  ...
  updateTracks(currentTime: number): {
    ...
    this.tracks.forEach((track, trackId) => {
      ...
      if (timeSinceUpdate > 0) {
        if ((this.fusionMode === 'EKF' || this.fusionMode === 'KALMAN') && track.kalmanState) {
          // EKF 모드: 칼만 필터로 예측
          track.kalmanState = this.ekf.predict(track.kalmanState, timeSinceUpdate);

          // 칼만 상태에서 위치/속도 추출
          const predictedPos = this.ekf.getPosition(track.kalmanState);
          const predictedVel = this.ekf.getVelocity(track.kalmanState);

          track.position = predictedPos;
          track.velocity = predictedVel;
          track.positionUncertainty = this.ekf.getPositionUncertainty(track.kalmanState);
          track.acceleration = this.ekf.getAcceleration(track.kalmanState);

          // 칼만 상태 유효성 체크
          if (!this.ekf.isStateValid(track.kalmanState)) {
            track.existenceProb *= 0.8; // 불확실성 증가 시 존재 확률 감소
          }
        } else {
          // 기본 모드: 속도 기반 선형 예측
          track.position = {
  ...
  private updateTrackWithObservation(
    ...
    // (B) 위치/속도 융합 - 모드에 따라 분기
    if (this.fusionMode === 'EKF' || this.fusionMode === 'KALMAN') {
      track = this.updatePositionVelocityWithEKF(track, obs, deltaTime, currentTime);
    } else {
  ...
  /**
   * (B-EKF) Extended Kalman Filter를 사용한 위치/속도 융합
   */
  private updatePositionVelocityWithEKF(
    track: ExtendedFusedTrack,
    obs: SensorObservation,
    deltaTime: number,
    currentTime: number
    ...
    if (!track.kalmanState) {
      track.kalmanState = this.ekf.createInitialState(
        track.position,
        track.velocity,
        currentTime
      );
    ...
    // 예측 단계: 시간 경과에 따른 상태 예측
    if (deltaTime > 0.01) {
      track.kalmanState = this.ekf.predict(track.kalmanState, deltaTime);
    }

    // 관측치를 칼만 관측 형식으로 변환
    const kalmanObs = this.convertToKalmanObservation(obs, currentTime);

    if (kalmanObs) {
      // 업데이트 단계: 센서 관측치로 상태 보정
      track.kalmanState = this.ekf.update(track.kalmanState, kalmanObs);

      // 기동 감지 시 프로세스 노이즈 적응
      track.kalmanState = this.ekf.adaptProcessNoise(track.kalmanState);
    }

    // 칼만 상태에서 위치/속도 추출
    const newPosition = this.ekf.getPosition(track.kalmanState);
    const newVelocity = this.ekf.getVelocity(track.kalmanState);
    const posUncertainty = this.ekf.getPositionUncertainty(track.kalmanState);
    const acceleration = this.ekf.getAcceleration(track.kalmanState);
  ...
  /**
   * SensorObservation을 KalmanObservation으로 변환
   */
  private convertToKalmanObservation(
    obs: SensorObservation,
    time: number
  ): KalmanObservation | null {
    // 방위각 변환 (도 → 라디안)
    const bearingRad = obs.bearing !== null ? (obs.bearing * Math.PI) / 180 : undefined;

    switch (obs.sensor) {
      case 'RADAR':
        if (obs.range !== null && bearingRad !== undefined) {
          return {
            sensor: 'RADAR',
            time,
            range: obs.range,
            bearing: bearingRad,
            altitude: obs.altitude ?? 100,
            radialVelocity: obs.metadata?.radialVelocity,
            confidence: obs.confidence,
          };
        }
        break;
  ...
  updateBasePosition(position: TrackPosition): void {
    ...
    this.ekf.updateBasePosition(position);
  }
   ...
   * EKF 설정 업데이트
   */
  updateEKFConfig(config: Partial<typeof DEFAULT_KALMAN_CONFIG>): void {
    this.ekf.updateConfig(config);
  }

  /**
   * 트랙의 위치 불확실성 반환 (EKF 모드에서만 유효)
   */
  getTrackUncertainty(trackId: string): { x: number; y: number; z: number } | null {
    const track = this.tracks.get(trackId) as ExtendedFusedTrack;
    if (track?.kalmanState) {
      return this.ekf.getPositionUncertainty(track.kalmanState);
    }
    return null;
  }

  /**
   * 트랙의 기동 강도 반환 (EKF 모드에서만 유효)
   */
  getTrackManeuverIntensity(trackId: string): number | null {
    const track = this.tracks.get(trackId) as ExtendedFusedTrack;
    if (track?.kalmanState) {
      return this.ekf.getManeuverIntensity(track.kalmanState);
    }
    return null;
  }
}
