...
import { FusedTrack, TrackPosition, TrackVelocity, ClassificationInfo, ExtendedFusedTrack } from './types';
import {
  ThreatEvaluationMode,
...
export interface UnifiedThreatResult {
  ...
  level: ThreatLevel;
  /** 평가 모드 */
  mode: ThreatEvaluationMode;
  /** ETA 정보 (동적 모드에서만) */
  etaInfo?: ETAInfo;
  /** 동적 요소 (동적 모드에서만) */
  dynamicFactors?: DynamicThreatFactors;
  /** 정적 요소 분해 (정적 모드에서만) */
  staticBreakdown?: ThreatScoreBreakdown;
}

/**
 * 통합 위협 평가 시스템
 *
 * 정적(Baseline) 및 동적(ETA 기반) 평가 모드 지원
 * 논문에서 Baseline과 제안 방식을 비교하기 위한 구조
 */
export class UnifiedThreatEvaluator {
  private mode: ThreatEvaluationMode = 'STATIC';
  private staticConfig: ThreatScoreConfig;
  private staticWeights: ThreatScoreWeights;
  private dynamicEvaluator: DynamicThreatEvaluator;

  constructor(
    basePosition: TrackPosition,
    config?: Partial<ThreatScoreConfig>,
    weights?: Partial<ThreatScoreWeights>
  ) {
    this.staticConfig = { ...DEFAULT_THREAT_SCORE_CONFIG, ...config };
    this.staticWeights = { ...DEFAULT_THREAT_WEIGHTS, ...weights };
    this.dynamicEvaluator = new DynamicThreatEvaluator(basePosition, {
      ...DEFAULT_DYNAMIC_THREAT_CONFIG,
      ...config,
    });
  }
   ...
   * 평가 모드 설정
   */
  setMode(mode: ThreatEvaluationMode): void {
    this.mode = mode;
  }
  ...
  evaluate(
    ...
    switch (this.mode) {
      case 'STATIC':
        return this.evaluateStatic(track);
      case 'DYNAMIC_ETA':
        return this.evaluateDynamicETA(track, currentTime);
      case 'BAYESIAN':
        // TODO: 베이지안 평가 구현 (비교용)
        return this.evaluateStatic(track);
      case 'DEMPSTER_SHAFER':
        // TODO: Dempster-Shafer 평가 구현 (비교용)
        return this.evaluateStatic(track);
      default:
        return this.evaluateStatic(track);
    }
   ...
   * 정적 평가 (Baseline)
   */
  private evaluateStatic(track: FusedTrack): UnifiedThreatResult {
    const score = computeThreatScore(track, this.staticConfig, this.staticWeights);
    const breakdown = computeThreatScoreBreakdown(track, this.staticConfig, this.staticWeights);

    return {
      score,
      level: getThreatLevel(score),
      mode: 'STATIC',
      staticBreakdown: breakdown,
    };
  }
   ...
   * 동적 ETA 기반 평가 (제안 방식)
   */
  private evaluateDynamicETA(
    track: FusedTrack | ExtendedFusedTrack,
    currentTime: number
  ): UnifiedThreatResult {
    // 히스토리 업데이트 (정적 점수 기준으로)
    const staticScore = computeThreatScore(track, this.staticConfig, this.staticWeights);
    this.dynamicEvaluator.updateThreatHistory(track.id, staticScore, currentTime);

    // 동적 점수 계산
    const dynamicScore = this.dynamicEvaluator.computeDynamicThreatScore(track, currentTime);
    const breakdown = this.dynamicEvaluator.computeDynamicThreatBreakdown(track, currentTime);

    return {
      score: dynamicScore,
      level: getThreatLevel(dynamicScore),
      mode: 'DYNAMIC_ETA',
      etaInfo: breakdown.etaInfo,
      dynamicFactors: {
        existenceScore: breakdown.existenceScore,
        ...
        armedScore: breakdown.armedScore,
        etaScore: breakdown.etaScore,
        threatDerivative: breakdown.threatDerivative,
        trajectoryScore: breakdown.trajectoryScore,
  ...
  evaluateAllModes(
    track: FusedTrack | ExtendedFusedTrack,
    currentTime: number
  ): Map<ThreatEvaluationMode, UnifiedThreatResult> {
    const results = new Map<ThreatEvaluationMode, UnifiedThreatResult>();

    // 정적 평가
    results.set('STATIC', this.evaluateStatic(track));

    // 동적 ETA 평가
    results.set('DYNAMIC_ETA', this.evaluateDynamicETA(track, currentTime));
  ...
  clearTrackHistory(trackId: string): void {
    this.dynamicEvaluator.clearTrackHistory(trackId);
  }
  ...
  clearAllHistories(): void {
    this.dynamicEvaluator.clearAllHistories();
  }
  ...
  updateBasePosition(position: TrackPosition): void {
    this.staticConfig.basePosition = position;
    this.dynamicEvaluator.updateBasePosition(position);
  }
}
 ...
 * 비교 분석 결과
 */
export interface ThreatEvaluationComparison {
  trackId: string;
  timestamp: number;
  staticScore: number;
  dynamicScore: number;
  ...
  recommendation: 'ENGAGE' | 'MONITOR' | 'IGNORE';
}

/**
 * 두 평가 모드 비교 분석
 */
export function compareThreatEvaluations(
  staticResult: UnifiedThreatResult,
  dynamicResult: UnifiedThreatResult,
  trackId: string,
  timestamp: number
): ThreatEvaluationComparison {
  const scoreDiff = dynamicResult.score - staticResult.score;
  const eta = dynamicResult.etaInfo?.eta ?? null;
  const isApproaching = dynamicResult.etaInfo?.isApproaching ?? false;

  // 교전 권고 결정
  let recommendation: 'ENGAGE' | 'MONITOR' | 'IGNORE';
  if (dynamicResult.score >= 70) {
    recommendation = 'ENGAGE';
  } else if (dynamicResult.score >= 40) {
  ...
  return {
    ...
    dynamicScore: dynamicResult.score,
    scoreDifference: scoreDiff,
    etaSeconds: eta !== null && isFinite(eta) ? eta : null,
    isApproaching,
    recommendation,
  };
}

// ThreatEvaluationMode re-export for convenience
export { ThreatEvaluationMode } from './dynamicThreat';
