...
/**
 * 칼만 필터 상태
 * 8차원 상태 벡터: [px, py, pz, vx, vy, vz, ax, ay]
 */
...
export interface KalmanFilterConfig {
  /** 프로세스 노이즈 */
  processNoise: {
    position: number;     // 위치 노이즈 (m)
    velocity: number;     // 속도 노이즈 (m/s)
    acceleration: number; // 가속도 노이즈 (m/s^2)
  };
  /** 관측 노이즈 */
  measurementNoise: {
    radar: { range: number; bearing: number; altitude: number; radialVel: number };
    audio: { bearing: number };
    eo: { range: number; bearing: number; altitude: number };
  };
  /** 초기 공분산 대각 요소 */
...
/**
 * 확장된 FusedTrack (칼만 필터 상태 포함)
 */
export interface ExtendedFusedTrack extends FusedTrack {
  /** 칼만 필터 상태 (EKF 모드에서 사용) */
