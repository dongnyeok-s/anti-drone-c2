...
import {
  InterceptProbabilityCalculator,
  ProbabilityModelMode,
  InterceptSituation,
  ExtendedProbabilityFactors,
} from './interceptProbability';
...
export interface ExtendedInterceptorDrone extends InterceptorDrone {
  ...
  // 확률 모델 관련 (Phase 3)
  probabilityMode: ProbabilityModelMode;  // 확률 계산 모드
  lastInterceptProbability?: number;      // 마지막 계산된 요격 확률
  probabilityFactors?: ExtendedProbabilityFactors;  // 확률 요소 상세
}
...
// 확률 모델 타입 재export
export { ProbabilityModelMode, InterceptSituation, ExtendedProbabilityFactors };

// 전역 확률 계산기 인스턴스
const probabilityCalculator = new InterceptProbabilityCalculator('SIMPLE');
...
export function createInterceptor(
  ...
  probabilityMode: ProbabilityModelMode = 'SIMPLE'  // 기본값: 단순 모델
  ...
  return {
    ...
    probabilityMode,
    lastInterceptProbability: undefined,
    probabilityFactors: undefined,
  };
}
 ...
 * 요격 상황 생성
 */
function createInterceptSituation(
  interceptor: ExtendedInterceptorDrone,
  target: HostileDrone,
  method: InterceptMethod
): InterceptSituation {
  return {
    interceptorPosition: interceptor.position,
    interceptorVelocity: interceptor.velocity,
    targetPosition: target.position,
    targetVelocity: target.velocity,
    targetIsEvading: target.isEvading,
    targetSizeClass: (target as any).sizeClass ?? 'MEDIUM',
    method,
    // 칼만 필터에서 온 정보가 있으면 사용
    positionUncertainty: undefined,
    trackQuality: undefined,
    maneuverIntensity: target.isEvading ? 0.7 : 0.3,
  };
}

/**
 * 요격 확률 계산 (모드에 따라 단순/로지스틱 선택)
 */
function calculateMethodProbability(
  ...
  const situation = createInterceptSituation(interceptor, target, method);

  // 확률 계산기 모드 설정
  probabilityCalculator.setMode(interceptor.probabilityMode);

  // 확률 계산
  const prob = probabilityCalculator.calculate(situation);
 ...
 * 요격 확률 상세 분석 (디버깅/로깅용)
 */
function calculateMethodProbabilityWithAnalysis(
  interceptor: ExtendedInterceptorDrone,
  target: HostileDrone,
  method: InterceptMethod
): { probability: number; factors: ExtendedProbabilityFactors } {
  const situation = createInterceptSituation(interceptor, target, method);

  // 로지스틱 모드로 상세 분석
  const analyzer = new InterceptProbabilityCalculator('LOGISTIC');
  const analysis = analyzer.analyzeFactors(situation);

  return {
    probability: analysis.logisticProb,
    factors: analysis.extended,
  };
}

/**
 * 레거시 확률 계산 (기존 시스템 호환용)
 */
function calculateMethodProbabilityLegacy(
  interceptor: ExtendedInterceptorDrone,
  target: HostileDrone,
  method: InterceptMethod
): number {
 ...
 * 확률 모델 모드 설정
 */
export function setProbabilityMode(
  interceptor: ExtendedInterceptorDrone,
  mode: ProbabilityModelMode
): ExtendedInterceptorDrone {
  return {
    ...interceptor,
...
/**
 * 전역 확률 계산기 모드 설정 (모든 인터셉터에 영향)
 */
export function setGlobalProbabilityMode(mode: ProbabilityModelMode): void {
  probabilityCalculator.setMode(mode);
}

/**
 * 현재 요격 확률 계산 및 저장
 */
export function updateInterceptProbability(
  interceptor: ExtendedInterceptorDrone,
  target: HostileDrone
): ExtendedInterceptorDrone {
  ...
  const analysis = calculateMethodProbabilityWithAnalysis(
    interceptor,
    target,
    interceptor.method
  );
  ...
  return {
    ...interceptor,
    lastInterceptProbability: analysis.probability,
    probabilityFactors: analysis.factors,
  };
}
 ...
 * 요격 확률 비교 분석 (모든 모드)
 */
export function compareInterceptProbabilities(
  interceptor: ExtendedInterceptorDrone,
  target: HostileDrone,
  method: InterceptMethod
): {
  ...
  const situation = createInterceptSituation(interceptor, target, method);
  const analyzer = new InterceptProbabilityCalculator('LOGISTIC');
  const allModes = analyzer.calculateAllModes(situation);
  const analysis = analyzer.analyzeFactors(situation);

  return {
    simple: allModes.get('SIMPLE') ?? 0,
    logistic: allModes.get('LOGISTIC') ?? 0,
    monteCarlo: allModes.get('MONTE_CARLO') ?? 0,
    factors: analysis.extended,
  };
}
